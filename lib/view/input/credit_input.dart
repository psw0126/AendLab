...
class CreditInputPage extends StatefulWidget {
  ...
  @override
  _CreditInputPageState createState() => _CreditInputPageState();
}
...
class _CreditInputPageState extends State<CreditInputPage> {
  ...
  String inputBarcode = '';
  String URL = '';
  bool leitorexterno = false;
  TpField tpfield = TpField.none;
  var pdaKey;

  final _formKey = GlobalKey<FormState>();
  final TercodeController = TextEditingController(); //바코드
  ...
  final TerbqtController = TextEditingController(); //전산재고
  ...
  setTerqtytxt(String key) {
    if (tpfield == TpField.none) {
      ...
      TerqtyController.text = TerqtyController.text + key.format(accessType == "4" ? FilteringTextInputFormatter.allow(RegExp(r'[0-9.-]')) : FilteringTextInputFormatter.allow(RegExp(r'[0-9]')));
    }
  }
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                          ...
                          child: Text('외상매입입력', style: TextStyle(
                              ...
                              child: Row(
                                ...
                                : SizedBox(width: 0),
                                  Container(
                                        ...
                                        child: DropdownButtonFormField(
                                          ...
                                          onChanged: (value) {
                                            setState(() async {
                                              ...
                                              pref.setString('tax', selectTax.toString());
                                            });
                                          },
                                          items: [
                                            DropdownMenuItem(
                                      ...
                                      //   // value: selectTax.toString().isEmpty ? '' : selectTax,
                                      //   value: TaxList.values,
                                      //   hint: Text('선택'),
                                      //   items: TaxList.values.map((item){
                                      //     return DropdownMenuItem(
                                      //       key: ValueKey(item.ko),
                                      //       value: item.id,
                                      //       child: Text(item.ko),
                                      //       onTap: () async {
                                      //         print(TaxList.values);
                                      //         print(item.id);
                                      //         SharedPreferences pref = await SharedPreferences.getInstance();
                                      //         pref.setString('tax', item.id.toString());
                                      //       },
                                      //     );
                                      //   }).toList(),
  ...
  Widget _Path() {
    ...
    return InkWell(
      ...
      onTap: () async {
          ...
          try {
            http.Response response = await http.patch(Uri.http(URL, 'v1/hand/terminal'),
                body: json.encode({
                  "str_code": "001",
            ...
            if (TernameController.text != '미등록상품' && TerqtyController.text == "0") {
              ...
              alert(context, '저장완료', '확인', onTap: () {
                Modular.to.popAndPushNamed(RouteMap.CREDIT_INPUT.page);
              });
            }
          } catch (e) {
            print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('CreditType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    if((scanBarcode == TercodeController.text) && (pref.getString('CreditType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if(TerqtyController.text == pdaKey){
          TerqtyController.text = handvalue[0]['itm_qty'].toString();
        }
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('CreditType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
    ...
    // if((inputBarcode == TercodeController.text) && (pref.getString('CreditType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
    //   showDialog(
    //       context: context,
    //       barrierDismissible: true,
    //       builder: (BuildContext ctx) {
