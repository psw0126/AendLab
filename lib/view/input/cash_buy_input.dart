...
class CashBuyInputPage extends StatefulWidget {
  @override
  _CashBuyInputPageState createState() => _CashBuyInputPageState();
}

class _CashBuyInputPageState extends State<CashBuyInputPage> {
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                          ...
                          child: Text('현금 매입 입력', style: TextStyle(
                                ...
                                try {
                                  http.Response response = await http.patch(Uri.http(URL,'v1/hand/terminal'),
                                      body: json.encode({
                                        ...
                                        "ter_type": pref.getString('CashBuyType'),
                                  ...
                                  if (TernameController.text != '미등록상품' && TerqtyController.text == "0") {
                                    ...
                                    alert(context, '저장완료', '확인', onTap: () {
                                      Modular.to.popAndPushNamed(RouteMap.CASH_BUY_INPUT.page);
                                    });
                                  }
                                } catch (e) {
                                  print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    var params = {
      ...
      'str_code': '001',
      'date': formatDate,
      'type': pref.getString('CashBuyType'),
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('CashBuyType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    if((scanBarcode == TercodeController.text) && (pref.getString('CashBuyType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('CashBuyType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
  ...
  Future Title() async {
    ...
    return pref.getString('CashTitle');
  }
