...
class _OrderInputPageState extends State<OrderInputPage> {
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                          ...
                          child: Text('발주 입력', style: TextStyle(
                                ...
                                try {
                                  http.Response response = await http.patch(
                                      ...
                                      body: json.encode({
                                        ...
                                        "ter_type": pref.getString('OrderType'),
                                  ...
                                  if (TernameController.text != '미등록상품' && TerqtyController.text == "0") {
                                    ...
                                    alert(context, '저장완료', '확인', onTap: () {
                                      Modular.to.popAndPushNamed(RouteMap.ORDERINPUT.page);
                                    });
                                  }
                                } catch (e) {
                                  print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('OrderType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    if((scanBarcode == TercodeController.text) && (pref.getString('OrderType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      ...
      return setState(() {
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('OrderType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
  ...
  Future Title() async {
    final SharedPreferences pref = await SharedPreferences.getInstance();
    accessType = pref.getString('access_option');
    return pref.getString('OrderTitle');
  }
