...
class CashReturnInputPage extends StatefulWidget {
  @override
  _CashReturnInputPageState createState() => _CashReturnInputPageState();
}

class _CashReturnInputPageState extends State<CashReturnInputPage> {
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                          ...
                          child: Text('현금 반품 입력', style: TextStyle(
                                ...
                                try {
                                  http.Response response = await http.patch(Uri.http(URL,'v1/hand/terminal'),
                                      body: json.encode({
                                        ...
                                        "ter_type": pref.getString('CashReturnType'),
                                  ...
                                  if (TernameController.text != '미등록상품' && TerqtyController.text == "0") {
                                    ...
                                    alert(context, '저장완료', '확인', onTap: () {
                                      Modular.to.popAndPushNamed(RouteMap.CASH_RETURN_INPUT.page);
                                    });
                                  }
                                } catch (e) {
                                  print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    var params = {
      ...
      'str_code': '001',
      'date': formatDate,
      'type': pref.getString('CashReturnType'),
      'barcode': _scanBarcode,
    };
    http.Response response = await http.get(Uri.http(URL, 'v1/item/p01', params),
        headers: {
          "Content-Type": "application/json",
          "Accept": "application/json",
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('CashReturnType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    if((scanBarcode == TercodeController.text) && (pref.getString('CashReturnType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('CashReturnType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
  ...
  Future Title() async {
    ...
    return pref.getString('CashReturnTitle');
  }
...
Future<bool> _onBackPressd() async {
  final SharedPreferences pref = await SharedPreferences.getInstance();
  pref.remove('ter_vencd');
  return pref.remove('CashReturnTitle');
}
