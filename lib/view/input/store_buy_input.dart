...
class StoreBuyInputPage extends StatefulWidget {
  @override
  _StoreBuyInputPageState createState() => _StoreBuyInputPageState();
}

class _StoreBuyInputPageState extends State<StoreBuyInputPage> {
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                          ...
                          child: Text('점간 매입 입력', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold),)
                      ),
                      Container(
                          child: FutureBuilder(
                            future: Title(),
                                ...
                                try {
                                  http.Response response = await http.patch(
                                      ...
                                      body: json.encode({
                                        ...
                                        "ter_type": pref.getString('StoreBuyType'),
                                  ...
                                  if(TerqtyController.text == "0") {
                                    ...
                                    alert(context, '저장완료', '확인', onTap: () {
                                      Modular.to.popAndPushNamed(RouteMap.STORE_BUY_INPUT.page);
                                    });
                                  }
                                } catch (e) {
                                  print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    var params = {
      ...
      'str_code': '001',
      'date': formatDate,
      'type': pref.getString('StoreBuyType'),
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('StoreBuyType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    if((scanBarcode == TercodeController.text) && (pref.getString('StoreBuyType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('StoreBuyType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
        ...
        // if((scanBarcode == TercodeController.text) && (pref.getString('StoreBuyType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
        //   showDialog(
        //       context: context,
        //       barrierDismissible: true,
        //       builder: (BuildContext ctx) {
  ...
  Future Title() async {
    ...
    access = pref.getString('access_type');
    return pref.getString('StoreTitle');
  }
  ...
  //   var params = {
  //     'user_id': pref.getString('user_id'),
  //     'str_code': pref.getString('ter_vencd'),
  //     'date': formatDate,
  //     'type': pref.getString('StoreBuyType'),
  //     'barcode': pref.getString('barcode'),
  //   };
  //   http.Response response = await http.get(
  //       Uri.http('aend.iptime.org:7020', 'v1/item/p01', params),
  //       headers: {
  //         "Content-Type": "application/json",
  //         "Accept": "application/json",
  //         "Authorization": "bearer ${pref.getString('access_token')}"
  ...
  //     _value = _valueList;
  //     print(_value);
  //     // select_value = _valueList[0]['itm_tax'];
  //   });
  // }
}
enum TaxList {
