...
class SalesInsertPage extends StatefulWidget {


  @override
  _SalesInsertPageState createState() => _SalesInsertPageState();
}

class _SalesInsertPageState extends State<SalesInsertPage> {
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                          ...
                          child: Text('매출등록 입력', style: TextStyle(
                            ...
                            InkWell(
                              ...
                              onTap: () async {
                                ...
                                try {
                                  ...
                                  }else if(TernameController.text == "미등록상품"){
                                    alert(context, '미등록상품입니다.', '확인');
                                  }
                                  else if (decodeResponse['error'] == "") {
                                    alert(context, '저장완료', '확인', onTap: () {
                                      Modular.to.popAndPushNamed(RouteMap.SALES_INSERT.page);
                                    });
                                  }
                                } catch (e) {
                                  print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    var params = {
      ...
      'str_code': '001',
      'date': formatDate,
      'type': pref.getString('SalesInsertType'),
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('SalesInsertType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    if((scanBarcode == TercodeController.text) && (pref.getString('SalesInsertType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('SalesInsertType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
  ...
  Future Title() async {
    ...
    return pref.getString('SalesTitle');
  }
