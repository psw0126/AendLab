...
class CommonUpdatePage extends StatefulWidget {
  String CommonUpdateType = '';
  CommonUpdatePage({Key? key, required this.CommonUpdateType}) : super(key: key);

  @override
  _CommonUpdatePageState createState() => _CommonUpdatePageState();
}

class _CommonUpdatePageState extends State<CommonUpdatePage> {
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                        ...
                        child: Text('일반상품 수정', style: TextStyle(
                              ...
                              try {
                                http.Response response = await http.patch(
                                    ...
                                    body: json.encode({
                                      ...
                                      "ter_type": pref.getString('CommonUpdateType'),
                                ...
                                print(decodeResponse);
                                 if (decodeResponse['error'] == "") {
                                  alert(context, '저장완료', '확인', onTap: () {
                                    Modular.to.popAndPushNamed(RouteMap.COMMON_UPDATE.page);
                                  });
                                }
                              } catch (e) {
                                print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    pref.setString('CommonUpdateType', widget.CommonUpdateType);
    ...
    var params = {
      ...
      'str_code': '001',
      'date': formatDate,
      'type': pref.getString('CommonUpdateType'),
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('CommonUpdateType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    if((scanBarcode == TercodeController.text) && (pref.getString('CommonUpdateType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('CommonUpdateType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
