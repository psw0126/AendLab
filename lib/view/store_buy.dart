...
class StoreBuyPage extends StatefulWidget {
  String StoreBuyType = '';
  StoreBuyPage({Key? key, required this.StoreBuyType}) : super(key: key);

  @override
  _StoreBuyPageState createState() => _StoreBuyPageState();
}

class _StoreBuyPageState extends State<StoreBuyPage> {
              ...
              Column(
                children: [
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          return Pda == true && Phone == false ? Container(
                            ...
                            child: Text('점간매입 메뉴', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          ) : Pda == false && Phone == true ? Container(
                            margin: EdgeInsets.symmetric(vertical: 30),
                            child: Text('점간매입 메뉴', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold)),
                          ) : Container(
                            margin: EdgeInsets.symmetric(vertical: 20),
                            child: Text('점간매입 메뉴', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          );
                  ...
                  InkWell(
                    ...
                    onTap: () async {
                      final SharedPreferences pref = await SharedPreferences.getInstance();
                      pref.setString('StoreBuyType', widget.StoreBuyType);
                      print('확인');
                      print(pref.getString('StoreBuyType'));
                      Modular.to.pushNamed(RouteMap.STORE_BUY_ENTER.page);
                    },
                  ),
                  InkWell(
                    child: Container(
                    ...
                    onTap: () async {
                      ...
                      print('확인');
                      print(pref.getString('StoreBuyType'));
                      Modular.to.pushNamed(RouteMap.STORE_BUY_CHECK.page);
                    },
                  ),
                ],
              ),
