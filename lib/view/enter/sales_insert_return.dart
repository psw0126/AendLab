...
class SalesInsertReturnPage extends StatefulWidget {
  String SalesReturnType = '';
  SalesInsertReturnPage({Key? key, required this.SalesReturnType}) : super(key: key);

  @override
  _SalesInsertReturnPageState createState() => _SalesInsertReturnPageState();
}

class _SalesInsertReturnPageState extends State<SalesInsertReturnPage> {
          ...
          child: Column(
            children: [
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          return Pda == true && Phone == false ? Container(
                            ...
                            child: Text('매출반품 입력', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          ) : Pda == false && Phone == true ? Container(
                            margin: EdgeInsets.symmetric(vertical: 30),
                            child: Text('매출반품 입력', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold)),
                          ) : Container(
                            margin: EdgeInsets.symmetric(vertical: 20),
                            child: Text('매출반품 입력', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          );
              ...
              InkWell(
                ...
                onTap: () async {
                  final SharedPreferences pref = await SharedPreferences.getInstance();
                  pref.setString('SalesReturnType', widget.SalesReturnType);
                  print('확인');
                  print(pref.getString('SalesReturnType'));
                  if (_typeAheadController.text == '' || _typeAheadController.text == null) {
                    alert(context, '거래선를 입력해주세요', '확인');
                  }else{
                    Modular.to.pushNamed(RouteMap.SALES_RETURN.page);
                  }
                  // if (_typeAheadController.text == '' || _typeAheadController.text == null) {
                  //   alert(context, '거래선를 입력해주세요', '확인');
