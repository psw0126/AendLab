...
class InventoryEnterPage extends StatefulWidget {
  @override
  _InventoryEnterPageState createState() => _InventoryEnterPageState();
}

class _InventoryEnterPageState extends State<InventoryEnterPage> {
              ...
              child: Column(
                children: [
                  ...
                  Container(
                      ...
                      child: Text('재고 입력', style: TextStyle(
                            ...
                            try {
                              http.Response response = await http.patch(Uri.http(URL,'v1/hand/terminal'),
                                  body: json.encode({
                                    ...
                                    "ter_type": pref.getString('InventoryType'),
                              ...
                              if (TernameController.text != '미등록상품' && TerqtyController.text == "0") {
                                ...
                                alert(context, '저장완료', '확인', onTap: () {
                                  Modular.to.popAndPushNamed(RouteMap.INVENTORYENTER.page);
                                });
                              }
                            } catch (e) {
                              print('에러');
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    var params = {
      ...
      'str_code': '001',
      'date': formatDate,
      'type': pref.getString('InventoryType'),
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((scanBarcode == TercodeController.text) && (pref.getString('InventoryType') == decodeResponse[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
    ...
    print('바코드');
    print(_scanBarcode);
    if((scanBarcode == TercodeController.text) && (pref.getString('InventoryType') == _value[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
      showDialog(
          context: context,
          barrierDismissible: true,
          builder: (BuildContext ctx) {
  ...
  Future<void> handItemList() async {
    ...
    if(decodeResponse[0]['itm_name'] == '미등록상품') {
      return setState(() {
        ...
        if((inputBarcode == TercodeController.text || scanBarcode == TercodeController.text) && (pref.getString('InventoryType') == handvalue[0]['itm_type'].toString()) && (TerqtyController.text != 0.toString())) {
          showDialog(
              context: context,
              barrierDismissible: true,
              builder: (BuildContext ctx) {
