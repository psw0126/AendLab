...
class OrderEnterPage extends StatefulWidget {
  @override
  _OrderEnterPageState createState() => _OrderEnterPageState();
}

class _OrderEnterPageState extends State<OrderEnterPage> {
  String? selectOrder;
            ...
            children: [
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          return Pda == true && Phone == false ? Container(
                            ...
                            child: Text('발주 입력', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          ) : Pda == false && Phone == true ? Container(
                            margin: EdgeInsets.symmetric(vertical: 30),
                            child: Text('발주 입력', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold)),
                          ) : Container(
                            margin: EdgeInsets.symmetric(vertical: 20),
                            child: Text('발주 입력', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          );
              ...
              InkWell(
                ...
                onTap: (){
                  ...
                  //   alert(context, '거래선을 입력해주세요', '확인');
                  // }else if(_cnamevalue != _typeAheadController.text){
                  //   alert(context, '검색 후 리스트를 선택해주세요', '확인');
                  // }else if(_value == null){
                  //   alert(context, '거래선을 선택해주세요', '확인');
                  // }else {
                    Modular.to.pushNamed(RouteMap.ORDERINPUT.page);
                  // }
                },
              ),
            ],
...
//           onTap: () async {
//             SharedPreferences pref = await SharedPreferences.getInstance();
//             pref.setString('OrderTitle', value['ven_name']);
//           },
//         );
//       }).toList(),
//     ),
    ...
    return Container(
        ...
        child: TypeAheadField<Company?>(
          ...
          onSuggestionSelected: (Company? suggestion) async {
            ...
            pref.setString('OrderTitle', company.cname);
